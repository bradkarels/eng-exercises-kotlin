...
class CircularBufferTest {
    @Test
    fun `clear it all out front to back`() {
    ...
    @Test
    fun `clear it all out as you go`() {
        val buffer = CircularBuffer<Int>(3)
        buffer.write(666)
        buffer.clear()
        buffer.write(27)
    ...
    fun `overwrite replaces the oldest item remaining in buffer following a read`() {
        val buffer = CircularBuffer<Int>(3)
        buffer.write(1) // 1*, n, n
        buffer.write(2) // 1*, 2, n
        buffer.write(3) // 1*, 2, 3

        assertEquals(1, buffer.read())  // n, 2*, 3
        buffer.write(4) // 4, 2*, 3
        buffer.overwrite(5) //4, 5, 3*

...
//1 <= n <= 3
object CountAndSay {
    fun countAndSay(n: Int): String = doCountAndSay(n)

    fun doCountAndSay(n: Int, iter: Int = 1, rle: String = "1"): String {
        return if (iter >= n) rle
        else {
            return doCountAndSay(n, iter + 1, makeRLE(rle))
        }
    }

    fun makeRLE(v: String, r: String = ""): String {
        return if (v.isEmpty()) return r
        else {
            val first = v.first()
            val enc = v.takeWhile { it == first }.length
            makeRLE(v.drop(enc),  r + "$enc$first")
        }
    }
}